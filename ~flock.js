!function() {
    const a = {
      ...
      , i = "session-id";
    let e = "analytics_events";
    const r = {
        cookie: "cookie",
        localStorage: "localStorage",
        sessionStorage: "sessionStorage"
    };
    let n, c, t, o, A, s = r.cookie, u = {}, m = !0;
    if (document.currentScript) {
        if (o = document.currentScript.getAttribute("data-host"),
        n = document.currentScript.getAttribute("data-proxy"),
        c = document.currentScript.getAttribute("data-proxy-url"),
        t = document.currentScript.getAttribute("data-token"),
        ...
        e = document.currentScript.getAttribute("data-datasource") || e,
        s = document.currentScript.getAttribute("data-storage") || s,
        m = "false" !== document.currentScript.getAttribute("data-stringify-payload");
        for (const a of document.currentScript.attributes)
            a.name.startsWith("tb_") && (u[a.name.slice(3)] = a.value)
    }
    function l() {
    ...
    function d() {
        if ([r.localStorage, r.sessionStorage].includes(s)) {
            const a = s === r.localStorage ? localStorage : sessionStorage
              , e = a.getItem(i);
            if (!e)
                return null;
            let n = null;
            ...
            if ("object" != typeof n || null === n)
                return null;
            return (new Date).getTime() > n.expiry ? (a.removeItem(i),
            null) : n.value
        }
        return function() {
            let a = {};
    ...
    function g() {
        ...
        if ([r.localStorage, r.sessionStorage].includes(s)) {
            const e = {
              ...
              , n = JSON.stringify(e)
              , c = s === r.localStorage ? localStorage : sessionStorage;
            return s === c.setItem(i, n)
        }
        return function(a) {
            let i = `session-id=${a}; Max-Age=1800; path=/; secure`;
            A && (i += `; domain=${A}`),
            document.cookie = i
        }(a)
    }
    ...
    async function S(a, i) {
        let r, A;
        if (g(),
        c ? r = c : n ? r = `${n}/api/tracking` : o ? (o = o.replaceAll(/\/+$/gm, ""),
        r = `${o}/v0/events?name=${e}&token=${t}`) : r = `https://api.tinybird.co/v0/events?name=${e}&token=${t}`,
        m)
            A = p(i),
            A = Object.assign({}, JSON.parse(A), u),
            A = JSON.stringify(A);
        else {
            A = Object.assign({}, i, u);
            const a = p(A);
